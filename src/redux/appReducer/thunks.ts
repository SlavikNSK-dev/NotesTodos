import { TNote } from '../notesReducer/types';
import { TTodo } from '../todosReducer/types';
...
export const initApp = () => async (dispatch: any) => {
  ...
  const notes: TNote[] = [
    ...
    },
    {
      ...
      title: 'Note title asdasdasdasd asdas dasdasd asdasdasdasd',
    },
  ];
  const todos: TTodo[] = [
    {
  ...
  await dispatch(initNotes(notes));
  await dispatch(initTodos(todos));
  // Исткуственно сэмулируем задержку с сервера
  setTimeout(async () => {
    await dispatch(initAppReducerAC(true))
  }, 1000);
};
